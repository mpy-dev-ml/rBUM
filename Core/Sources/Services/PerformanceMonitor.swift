...
public actor PerformanceMonitor: PerformanceMonitorProtocol {
    ...
    public func endOperation(_ operationId: UUID, status: OperationStatus) async {
        ...
        if case let .failed(error) = status {
            ...
            logger.info(
                "Completed operation: \(operation.name)",
                metadata: ["duration": "\(endedOperation.duration ?? 0)ms"]
            )
        }
    }
    ...
    private func calculateStatistics(
        ...
        let durations = completedOperations.compactMap(\.duration)
        ...
        let avgCPU = cpuMetrics.isEmpty ? 0 : cpuMetrics.map(\.value).reduce(0, +) / Double(cpuMetrics.count)
        let peakCPU = cpuMetrics.map(\.value).max() ?? 0
        let avgSpeed = speedMetrics.isEmpty ?
            0 : speedMetrics.map(\.value).reduce(0, +) / Double(speedMetrics.count)
