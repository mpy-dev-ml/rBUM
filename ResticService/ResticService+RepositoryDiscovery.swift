...
private extension ResticService {
    ...
    func indexRepositoryContents(at url: URL) async throws {
        ...
        guard process.terminationStatus == 0,
              let snapshots = try JSONSerialization.jsonObject(with: snapshotData) as? [[String: Any]] else {
    ...
    private func indexSnapshot(snapshotID: String, at url: URL) async throws {
        ...
        guard process.terminationStatus == 0,
              let files = try JSONSerialization.jsonObject(with: listData) as? [[String: Any]] else {
