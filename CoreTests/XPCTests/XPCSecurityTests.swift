...
final class XPCSecurityTests: XCTestCase {
    ...
    func testXPCConnectionFailure() async throws {
        ...
        await XCTAssertThrowsError(try await securityService.validateXPCService()) { error in
    ...
    func testXPCPermissionValidation() async throws {
        ...
        await XCTAssertThrowsError(try await securityService.validateXPCService())
    }
    
    func testPermissionDenial() async throws {
        // Test permission denial
        mockXPC.shouldFailExecution = true
        await XCTAssertThrowsError(try await mockXPC.executeCommand("test", withBookmark: Data())) { error in
            XCTAssertTrue(error is SecurityError)
        }
    }
