...
final class ResticXPCServiceTests: XCTestCase {
    ...
    func testInitializeRepositoryFailure() async {
        ...
        // When/Then
        await XCTAssertThrowsError(try service.initializeRepository(at: invalidPath, password: testPassword)) { error in
    ...
    func testBackupFailure() async {
        ...
        // When/Then
        await XCTAssertThrowsError(try service.backup(
    ...
    func testRestore() async throws {
        ...
        _ = try await service.initializeRepository(at: repoURL.path, password: testPassword)
        let backupOutput = try await service.backup(
        ...
        // When
        let output = try await service.restore(
            repository: repoURL.path,
            destination: restoreURL.path,
            snapshot: snapshotId,
            password: testPassword
        )
    ...
    func testListSnapshotsFailure() async {
        ...
        // When/Then
        await XCTAssertThrowsError(try service.listSnapshots(
            repository: invalidRepo,
            password: testPassword
        )) { error in
            XCTAssertTrue(error is XPCError)
