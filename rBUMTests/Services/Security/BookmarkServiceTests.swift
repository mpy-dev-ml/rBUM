...
final class BookmarkServiceTests: XCTestCase {
    ...
    func testBookmarkStaleness() async throws {
        ...
        // When/Then
        await XCTAssertThrowsError(try service.validateBookmark(bookmark, for: testURL)) { error in
    ...
    func testAccessWithInvalidBookmark() async throws {
        ...
        // When/Then
        await XCTAssertThrowsError(try service.startAccessing(testURL, with: invalidBookmark)) { error in
            XCTAssertTrue(error is BookmarkError)
        }
        verifyLogMessages(
            mockLogger,
