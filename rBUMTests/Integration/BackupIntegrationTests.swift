...
final class BackupIntegrationTests: XCTestCase {
    ...
    func testBackupWorkflowWithFailures() async throws {
        ...
        await XCTAssertThrowsError(try backupService.initializeRepository(at: repoURL)) { error in
        ...
        await XCTAssertThrowsError(try securityService.validateAccess(to: sourceURL)) { error in
        ...
        mockBookmarkService.isValidBookmark = true
        mockXPCService.shouldFail = true
        await XCTAssertThrowsError(try backupService.backup(source: sourceURL, to: repoURL)) { error in
            XCTAssertTrue(error is ResticError)
