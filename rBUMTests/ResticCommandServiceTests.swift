...
final class ResticCommandServiceTests: XCTestCase {
    ...
    func testInitializeRepositoryFailure() async {
        ...
        // When/Then
        await XCTAssertThrowsError(try service.initializeRepository(at: testURL)) { error in
    ...
    func testBackupFilesFailure() async {
        ...
        // When/Then
        await XCTAssertThrowsError(try service.backup(source: sourceURL, to: repoURL)) { error in
            XCTAssertTrue(error is ResticError)
    ...
    func testRestoreFilesFailure() async {
        ...
        // When/Then
        await XCTAssertThrowsError(try service.restore(from: repoURL, to: targetURL, snapshot: "abc123")) { error in
            XCTAssertTrue(error is ResticError)
    ...
    func testListSnapshotsFailure() async {
        ...
        // When/Then
        await XCTAssertThrowsError(try service.listSnapshots(in: repoURL)) { error in
            XCTAssertTrue(error is ResticError)
    ...
    func testInvalidRepositoryURL() async {
        ...
        // When/Then
        await XCTAssertThrowsError(try service.initializeRepository(at: invalidURL)) { error in
            XCTAssertTrue(error is ResticError)
    ...
    func testMissingCredentials() async {
        ...
        // When/Then
        await XCTAssertThrowsError(try service.initializeRepository(at: repoURL)) { error in
            XCTAssertTrue(error is ResticError)
...
private class MockKeychainService: KeychainServiceProtocol {
    ...
    func getCredentials(for _: URL) async throws -> Credentials {
    ...
    func storeCredentials(_: Credentials, for _: URL) async throws {}

    func deleteCredentials(for _: URL) async throws {}
