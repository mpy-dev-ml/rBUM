...
final class KeychainCredentialsManager: KeychainCredentialsManagerProtocol {
    ...
    init(
        ...
        do {
            ...
            logger.error(
                "Failed to configure keychain sharing: \(error.localizedDescription)",
                file: #file,
                function: #function,
                line: #line
            )
    ...
    func delete(forId id: String) async throws {
        ...
        do {
            ...
            logger.error(
                "Failed to delete credentials: \(error.localizedDescription)",
                file: #file,
                function: #function,
                line: #line
            )
    ...
    func list() async throws -> [(repositoryId: String, credentials: RepositoryCredentials)] {
        ...
        do {
            ...
            logger.debug(
                "Successfully listed \(results.count) credentials",
                file: #file,
                function: #function,
                line: #line
            )
            ...
            logger.error(
                "Failed to list credentials: \(error.localizedDescription)",
                file: #file,
                function: #function,
                line: #line
            )
...
enum CredentialsError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case let .storeFailed(reason):
            "Failed to store credentials: \(reason)"
        case let .retrievalFailed(reason):
            "Failed to retrieve credentials: \(reason)"
        case let .deletionFailed(reason):
            "Failed to delete credentials: \(reason)"
        case let .listingFailed(reason):
            "Failed to list credentials: \(reason)"
        }
    }
}
...
final class AppKeychainCredentialsManager {
    ...
    init(serviceName: String = "dev.mpy.rBUM.repository") {
        manager = KeychainCredentialsManager(serviceName: serviceName)
    }
}
