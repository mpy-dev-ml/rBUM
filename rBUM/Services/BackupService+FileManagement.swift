...
extension BackupService {
    ...
    internal func scanSourceDirectory(_ url: URL) async throws -> [URL] {
        var files: [URL] = []
        ...
        if try await fileManager.isDirectory(at: url) {
            ...
            for fileURL in contents {
                if try await fileManager.isDirectory(at: fileURL) {
                    ...
                    files.append(contentsOf: try await scanSourceDirectory(fileURL))
                } else {
    ...
    internal func validateFileAccess(at url: URL) async throws -> Bool {
    ...
    internal func validateBackupDestination(_ url: URL) async throws -> Bool {
    ...
    internal func cleanupTemporaryFiles(at url: URL) async throws {
        guard try await fileManager.fileExists(at: url) else {
            return
        }
