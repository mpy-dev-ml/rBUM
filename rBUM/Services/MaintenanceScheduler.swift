...
final class MaintenanceScheduler {
    ...
    init(
        ...
        taskRecovery = MaintenanceTaskRecovery(
            logger: logger,
            configStore: store
        )
        ...
        schedules = try store.loadSchedules()
    ...
    private func checkSchedules() {
        queue.async { [weak self] in
            ...
            for (path, schedule) in schedules {
                ...
                if shouldRunMaintenance(schedule: schedule, lastRun: lastRuns[path], now: now) {
                    guard !runningMaintenance.contains(path) else {
                        logger.warning("Maintenance already running for: \(path)")
                        continue
                    }
                    ...
                    // Start maintenance
                    runningMaintenance.insert(path)
